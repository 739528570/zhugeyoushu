...
// 章节分割主函数
async function splitChapters(event) {
  try {
...
async function saveChaptersToDB({ docId, openid, chapters, totalChapters }) {
  ...
  // 先删除该文档已有的章节信息（如果有）
  await db.collection("chapters").where({ docId, openid }).remove();
...
async function processChapters({
  ...
  chapters.forEach((chapter) => {
    chapter.startPage = Math.floor(chapter.start / pageSize) + 1;
    chapter.endPage = Math.min(Math.ceil(chapter.end / pageSize), totalPages);
  });
  ...
  console.log(`文档 ${docId} 分章节完成，共 ${chapters.length} 章`);
}

// 辅助函数：从文件名提取标题（去除扩展名）
function getTitleFromFileName(fileName) {
  return fileName.replace(/\.(txt|epub|pdf)$/i, "");
}
exports.main = async (event, context) => {
  return await splitChapters(event)
}
